...
In the [first](https://supertype.ai/notes/fear-greed-index-part1/) and [second](https://supertype.ai/notes/fear-greed-index-part2/) part of this article series, we explored the concept of Fear and Greed Index, and walked through the theoretical and mathematical concepts used for the calculation. This part of the article will focus more on the technical and operation aspect of the data engineering process. We will begin by exploring the data sources, the considerations that rise from different data sources, and the challenges for automation that requires some workarounds.
...
2. For the data that is not yet available in the data warehouse, a good starting point for potential sources would be the current data sources currently consumed by our data pipeline. This is beneficial for some reason:
...
### 2. <a name="indonesian-bonds-data-1">Indonesian Bonds Data</a>:
    ...
    This API fetches data from PHEI.
    ...
    This API fetches data from Indonesia's 10Y bond yield from investing.com.
...
The previous approach worked well and is simple to understand, until we have the need for more granular control over the parameters in the function. Passing more and more parameters would make the code less elegant and less readable. To work around this, we finally decided to use OOP approach for the Fear and Greed Index calculation. This approach is used to encapsulate and abstract the parameters as state of the Fear and Greed Index "calculator". This state can then be changed accordingly, which is helpful when we need to compare results with different parameters.
...
* Intermediary indices weights\
    The weight of each intermediary indices' result towards the final Fear and Greed Index result. This is currently implemented for linear weights that sum up to 1.
